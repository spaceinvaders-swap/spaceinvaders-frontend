...
import { SPACEINVADERS_SPACE, ADMINS } from 'views/Voting/config'
  ...
  const response = await request(
    ...
    gql`
      query getProposals($first: Int!, $skip: Int!, $state: String!, $admins: [String]!) {
        proposals(first: $first, skip: $skip, where: { author_in: $admins, space_in: "${SPACEINVADERS_SPACE}", state: $state }) {
          id
        }
      }
    `,
