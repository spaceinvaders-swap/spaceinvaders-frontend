...
import { Heading, Text, Button, ArrowForwardIcon, Link, PageHeader, Pool } from '@spaceinvaders-swap/uikit'
...
const MigrationPage: React.FC<React.PropsWithChildren> = () => {
  ...
  const steps: Step[] = [
    {
      ...
      canHover: true,
      text: t('Unstake LP tokens and INVA from the old MasterChef contract.'),
    },
    {
      stepId: ProgressStepsType.STEP2,
      canHover: true,
      text: t('Stake LP tokens and INVA to the new MasterChef v2 contract.'),
    },
  ]
  ...
  const { data: invaPool, userDataLoaded } = useFetchUserPools(account)

  const v1Pools = useMemo(() => {
    const ifoPoolVault = { ...invaPool, vaultKey: VaultKey.IfoPool }
    const invaAutoVault = { ...invaPool, vaultKey: VaultKey.InvaVaultV1 }

    return [ifoPoolVault, invaAutoVault, invaPool]
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [invaPool]) as Pool.DeserializedPool<Token>[]
  ...
  return (
    <div ref={tableWrapperEl}>
      <PageHeader>
        ...
        <Link href="https://docs.spaceinvaders-swap.finance/code/migration/migrate-your-stakings" external>
