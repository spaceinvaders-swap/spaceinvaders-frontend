...
import { AutoRenewIcon, Button, Flex, PresentWonIcon, Text, useToast, Balance } from '@offsideswap/uikit'
...
const ClaimInnerContainer: React.FC<React.PropsWithChildren<ClaimInnerProps>> = ({ onSuccess, roundsToClaim }) => {
  ...
  const rotoReward = activeClaimData.rotoTotal
  const dollarReward = rotoReward.times(rotoPriceBusd)
  const rewardAsBalance = getBalanceAmount(rotoReward).toNumber()
    ...
    if (receipt?.status) {
      toastSuccess(
        ...
        <ToastDescriptionWithTx txHash={receipt.transactionHash}>
          {t('Your ROTO prizes for round %lotteryId% have been sent to your wallet', { lotteryId })}
