...
import { useInva, useLotteryV2Contract } from 'hooks/useContract'
...
import { BIG_ZERO, BIG_ONE_HUNDRED } from '@spaceinvaders-swap/utils/bigNumber'
...
const BuyTicketsModal: React.FC<React.PropsWithChildren<BuyTicketsModalProps>> = ({ onDismiss }) => {
  ...
  const stringifiedUserInva = userInva.toJSON()
  const memoisedUserInva = useMemo(() => new BigNumber(stringifiedUserInva), [stringifiedUserInva])
  ...
  const validateInput = useCallback(
    ...
    [limitNumberByMaxTicketsPerBuy, getTicketCostAfterDiscount, maxNumberTicketsPerBuyOrClaim, userInva],
  )

  useEffect(() => {
    const getMaxPossiblePurchase = () => {
      const maxBalancePurchase = memoisedUserInva.div(priceTicketInInva)
      const limitedMaxPurchase = limitNumberByMaxTicketsPerBuy(maxBalancePurchase)
      let maxPurchase

      // If the users' max INVA balance purchase is less than the contract limit - factor the discount logic into the max number of tickets they can purchase
  ...
  useEffect(() => {
    ...
    const costBeforeDiscount = priceTicketInInva.times(numberOfTicketsToBuy)
  ...
  }, [ticketsToBuy, priceTicketInInva, discountDivisor, getTicketCostAfterDiscount])
  ...
  const { isApproving, isApproved, isConfirmed, isConfirming, handleApprove, handleConfirm } =
    useApproveConfirmTransaction({
      onRequiresApproval: async () => {
        return requiresApproval(invaContractReader, account, lotteryContract.address)
      },
      onApprove: () => {
        return callWithGasPrice(invaContractApprover, 'approve', [lotteryContract.address, MaxUint256])
      },
  ...
  const getErrorMessage = () => {
    if (userNotEnoughInva) return t('Insufficient INVA balance')
  ...
  return (
    <StyledModal title={t('Buy Tickets')} onDismiss={onDismiss} headerBackground={theme.colors.gradientCardHeader}>
      ...
      <BalanceInput
        isWarning={account && (userNotEnoughInva || maxTicketPurchaseExceeded)}
        placeholder="0"
        value={ticketsToBuy}
        onUserInput={handleInputChange}
        currencyValue={
          invaPriceBusd.gt(0) &&
          `~${ticketsToBuy ? getFullDisplayBalance(priceTicketInInva.times(new BigNumber(ticketsToBuy))) : '0.00'} INVA`
        }
      />
      <Flex alignItems="center" justifyContent="flex-end" mt="4px" mb="12px">
        <Flex justifyContent="flex-end" flexDirection="column">
          {account && (userNotEnoughInva || maxTicketPurchaseExceeded) && (
      ...
      <Flex flexDirection="column">
        <Flex mb="8px" justifyContent="space-between">
          ...
          <Text color="textSubtle" fontSize="14px">
            {priceTicketInInva && getFullDisplayBalance(priceTicketInInva.times(ticketsToBuy || 0))} INVA
          </Text>
        </Flex>
        <Flex mb="8px" justifyContent="space-between">
          <Flex>
