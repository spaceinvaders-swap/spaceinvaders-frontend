...
import { Box, Flex, Heading, Skeleton, Balance } from '@offsideswap/uikit'
...
const Hero = () => {
  ...
  const {
    currentRound: { amountCollectedInRoto, status },
    isTransitioning,
  } = useLottery()

  const rotoPriceBusd = usePriceRotoBusd()
  const prizeInBusd = amountCollectedInRoto.times(rotoPriceBusd)
