...
import { Skeleton, useToast, useModal, Farm as FarmUI } from '@offsideswap/uikit'
...
export const ProxyHarvestActionContainer = ({ children, ...props }) => {
  ...
  const { onReward, onDone, proxyRotoBalance } = useProxyStakedActions(props.pid, lpContract)

  return children({ ...props, onReward, proxyRotoBalance, onDone })
}
...
export const HarvestAction: React.FunctionComponent<React.PropsWithChildren<HarvestActionProps>> = ({
  ...
  return (
    <FarmTableHarvestAction
      ...
      proxyRotoBalance={proxyRotoBalance}
