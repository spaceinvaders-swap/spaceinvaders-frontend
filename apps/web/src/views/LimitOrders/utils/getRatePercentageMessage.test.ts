import { JSBI, Price, ERC20Token } from '@spaceinvaders-swap/sdk'
...
const INVA = new ERC20Token(56, '0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82', 18, 'INVA', 'SpaceinvadersSwap Token')
...
const ONE_BUSD_PER_INVA = new Price(INVA, BUSD, EIGHTEEN_DECIMALS, ONE)
const FIVE_BUSD_PER_INVA = new Price(INVA, BUSD, EIGHTEEN_DECIMALS, FIVE)
const SEVEN_BUSD_PER_INVA = new Price(INVA, BUSD, EIGHTEEN_DECIMALS, SEVEN)
const ELEVEN_BUSD_PER_INVA = new Price(INVA, BUSD, EIGHTEEN_DECIMALS, ELEVEN)
const SEVEN_HUNDRED_BUSD_PER_INVA = new Price(INVA, BUSD, EIGHTEEN_DECIMALS, SEVEN_HUNDRED)
...
describe('limitOrders/utils/getRatePercentageMessage', () => {
  describe.each([
    [
      getRatePercentageDifference(SEVEN_BUSD_PER_INVA, ELEVEN_BUSD_PER_INVA),
      ['57.14% above market', PercentageDirection.ABOVE],
    ],
    [
      getRatePercentageDifference(SEVEN_BUSD_PER_INVA, FIVE_BUSD_PER_INVA),
      ['-28.57% below market', PercentageDirection.BELOW],
    ],
    [
      getRatePercentageDifference(SEVEN_BUSD_PER_INVA, SEVEN_HUNDRED_BUSD_PER_INVA),
      ['9,900% above market', PercentageDirection.ABOVE],
    ],
    [
      getRatePercentageDifference(SEVEN_BUSD_PER_INVA, ONE_BUSD_PER_INVA),
      ['-85.71% below market', PercentageDirection.BELOW],
    ],
    [
      getRatePercentageDifference(SEVEN_BUSD_PER_INVA, SEVEN_BUSD_PER_INVA),
