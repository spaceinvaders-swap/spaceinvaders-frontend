...
import { AutoRenewIcon, Button, Checkbox, Flex, InjectedModalProps, Text, useToast } from '@spaceinvaders-swap/uikit'
...
const PauseProfilePage: React.FC<React.PropsWithChildren<PauseProfilePageProps>> = ({ onDismiss, onSuccess }) => {
  ...
  const {
    costs: { numberInvaToReactivate },
  } = useGetProfileCosts()
  const { t } = useTranslation()
  const spaceinvadersProfileContract = useProfileContract()
  ...
  const handleDeactivateProfile = async () => {
    const receipt = await fetchWithCatchTxError(() => {
      return callWithGasPrice(spaceinvadersProfileContract, 'pauseProfile')
    })
    if (receipt?.status) {
      // Re-fetch profile
      refreshProfile()
  ...
  return (
    <>
      ...
      <Text as="p" color="textSubtle" mb="24px">
        {t('Cost to reactivate in the future: %cost% INVA', { cost: formatBigNumber(numberInvaToReactivate) })}
      </Text>
