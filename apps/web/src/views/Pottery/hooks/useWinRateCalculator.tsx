...
const useWinRateCalculator = ({ invaPrice, totalSupply }: WinRateCalculatorProps) => {
  ...
  const totalLockValueAsUSD = useMemo(() => {
    return new BigNumber(totalLockValue).times(invaPrice).toNumber()
  }, [totalLockValue, invaPrice])
  ...
  const setPrincipalFromTokenValue = (amount: string) => {
    const principalAsUsdBN = new BigNumber(amount).times(invaPrice)
  ...
  const setPrincipalFromUSDValue = (amount: string) => {
    const principalAsTokenBN = new BigNumber(amount).div(invaPrice)
  ...
  const setTargetWinRate = (percentage: string) => {
    const invaAmount = new BigNumber(percentage).dividedBy(100).times(totalLockValue)
    const principalAsToken = invaAmount.gt(0) ? invaAmount.toFixed(TOKEN_PRECISION) : '0.00'
    const principalAsUsdBN = new BigNumber(invaAmount).times(invaPrice)
    const principalAsUsdString = principalAsUsdBN.gt(0) ? principalAsUsdBN.toFixed(USD_PRECISION) : '0.00'
