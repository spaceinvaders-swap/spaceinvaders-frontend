...
import { Flex, Box, Text, TooltipText, useTooltip, Balance } from '@offsideswap/uikit'
...
import { remainTimeToNextFriday, calculateRotoAmount } from 'views/Pottery/helpers'
...
const Deposit: React.FC<React.PropsWithChildren> = () => {
  ...
  const { totalSupply, totalLockRoto, getStatus, totalLockedValue, maxTotalDeposit } = publicData
  ...
  const totalValueLocked = useMemo(() => {
    if (getStatus === PotteryDepositStatus.LOCK) {
      return getBalanceNumber(totalLockRoto)
    }
    return getBalanceNumber(totalLockedValue)
  }, [getStatus, totalLockRoto, totalLockedValue])
  ...
  const depositBalance = useMemo(() => {
    ...
    if (currentDeposit) {
      ...
      return calculateRotoAmount({ status, previewRedeem, shares, totalSupply, totalLockRoto })
    }

    return BIG_ZERO
  }, [userData, getStatus, currentDeposit, totalSupply, totalLockRoto])
    ...
    <Box>
      <Container>
        ...
        <Flex justifyContent="space-between">
          ...
          <Balance bold decimals={2} value={totalValueLocked} unit=" ROTO" />
        </Flex>
        <Flex justifyContent="space-between">
          <Text color="textSubtle">{t('Max. deposit cap')}</Text>
          <Balance bold decimals={2} value={getBalanceNumber(maxTotalDeposit)} unit=" ROTO" />
        </Flex>
      </Container>
      <CardAction>
