...
import { fetchInvaVaultFees, fetchPoolsPublicDataAsync, fetchInvaVaultPublicData } from 'state/pools'
...
const useGetTopPoolsByApr = (isIntersecting: boolean) => {
  ...
  useEffect(() => {
    const [invaPools, otherPools] = partition(pools, (pool) => pool.sousId === 0)
    const masterInvaPool = invaPools.filter((invaPool) => invaPool.vaultKey === VaultKey.InvaVault)
    const getTopPoolsByApr = (activePools: Pool.DeserializedPool<Token>[]) => {
      ...
      setTopPools([...masterInvaPool, ...sortedByApr.slice(0, 4)])
    }
