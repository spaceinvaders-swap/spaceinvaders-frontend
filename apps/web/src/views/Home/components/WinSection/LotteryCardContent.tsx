...
import { Flex, Text, Skeleton, Button, ArrowForwardIcon, Balance, NextLinkFromReactRouter } from '@spaceinvaders-swap/uikit'
...
const LotteryCardContent = () => {
  ...
  const invaPriceBusd = usePriceInvaBusd({ forceMainnet: true })
  ...
  const invaPrizesText = t('%invaPrizeInUsd% in INVA prizes this round', { invaPrizeInUsd: invaPriceBusd.toString() })
  const [pretext, prizesThisRound] = invaPrizesText.split(invaPriceBusd.toString())
  const amountCollectedInInva = currentLottery ? parseFloat(currentLottery.amountCollectedInInva) : null
  const currentLotteryPrize = amountCollectedInInva ? invaPriceBusd.times(amountCollectedInInva) : null
    ...
    <>
      <Flex flexDirection="column" mt="48px">
        ...
        <Text color="white" mb="40px">
          {t('Buy tickets with INVA, win INVA if your numbers match')}
        </Text>
      </Flex>
