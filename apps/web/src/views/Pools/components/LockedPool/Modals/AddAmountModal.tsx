...
import { Modal, Box, MessageText, Message, Checkbox, Flex, Text } from '@spaceinvaders-swap/uikit'
...
import { getBalanceNumber, getDecimalAmount, getBalanceAmount } from '@spaceinvaders-swap/utils/formatBalance'
      ...
      {!checkedState && (
        <Message variant="warning" mb="16px">
          <MessageText maxWidth="320px">
            {t(
              'Adding more INVA will renew your lock, setting it to remaining duration. Due to shorter lock period, benefits decrease. To keep similar benefits, extend your lock.',
            )}
          </MessageText>
        </Message>
      )}
...
const AddAmountModal: React.FC<React.PropsWithChildren<AddAmountModalProps>> = ({
  ...
  const usdValueStaked = useBUSDInvaAmount(lockedAmountAsBigNumber.toNumber())
  const usdValueNewStaked = useBUSDInvaAmount(totalLockedAmount)
  ...
  // if you locked for 1 week, then add inva without renew the extension, it's possible that remainingDuration + passedDuration less than 1 week.
  ...
  return (
    <RoiCalculatorModalProvider lockedAmount={lockedAmount}>
      <Modal title={t('Add INVA')} onDismiss={onDismiss} headerBackground={theme.colors.gradientCardHeader}>
        <Box mb="16px">
          <BalanceField
