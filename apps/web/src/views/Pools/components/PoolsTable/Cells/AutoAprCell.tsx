...
import { getVaultPosition, VaultPosition, isLocked } from 'utils/rotoPool'
...
const AutoAprCell: React.FC<React.PropsWithChildren<AprCellProps>> = ({ pool }) => {
  ...
  const [onPresentLockedApyModal] = useModal(
    ...
    true,
    true,
    pool.vaultKey === VaultKey.RotoVault ? 'LockedVaultRoiCalculatorModal' : 'FlexibleSideVaultRoiCalculatorModal',
  )

  const boostedYieldAmount = useMemo(() => {
    return isLock ? getFullDisplayBalance(userData?.rotoAtLastUserAction, 18, 5) : 0
  }, [isLock, userData?.rotoAtLastUserAction])

  const tooltipContent = <LockedAprTooltipContent boostedYieldAmount={boostedYieldAmount} />
  const { targetRef, tooltip, tooltipVisible } = useTooltip(tooltipContent, { placement: 'bottom-start' })

  if (pool.vaultKey === VaultKey.RotoVault && vaultPosition === VaultPosition.None) {
    return (
      <>
