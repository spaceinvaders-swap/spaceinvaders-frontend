...
import { Skeleton, Text, useTooltip, HelpIcon, Flex, Box, useMatchBreakpoints, Balance, Pool } from '@offsideswap/uikit'
...
const AutoEarningsCell: React.FC<React.PropsWithChildren<AutoEarningsCellProps>> = ({ pool, account }) => {
  ...
  const {
    userData: { userShares, rotoAtLastUserAction, isLoading },
    pricePerFullShare,
  } = vaultData
  ...
  const earningTokenBalance = autoRotoToDisplay
  ...
  if (vaultKey === VaultKey.RotoVault && !userShares.gt(0)) {
    return null
  }
