...
import { getOffsideBunniesAddress } from 'utils/addressHelpers'
...
export const useGetCollectionDistributionPB = () => {
  ...
  useEffect(() => {
    const fetchTokens = async () => {
      ...
      try {
        apiResponse = await getNftsFromCollectionApi(offsideBunniesAddress)
        if (!apiResponse) {
      ...
      const bunnyCountCalls = tokenIds.map((tokenId) => ({
        address: getOffsideBunniesAddress(),
        name: 'bunnyCount',
        params: [tokenId],
      }))
      try {
        const response = await multicallv2({ abi: offsideBunniesAbi, calls: bunnyCountCalls })
