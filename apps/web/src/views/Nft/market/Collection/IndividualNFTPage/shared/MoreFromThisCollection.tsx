...
import { ArrowBackIcon, ArrowForwardIcon, Box, IconButton, Text, Flex, useMatchBreakpoints } from '@spaceinvaders-swap/uikit'
...
const MoreFromThisCollection: React.FC<React.PropsWithChildren<MoreFromThisCollectionProps>> = ({
  ...
  const allSpaceinvadersBunnyNfts = useAllSpaceinvadersBunnyNfts(collectionAddress)
  ...
  let nftsToShow = useMemo(() => {
    return shuffle(
      allSpaceinvadersBunnyNfts
        ? allSpaceinvadersBunnyNfts.filter((nft) => nft.name !== currentTokenName)
        : collectionNfts?.filter((nft) => nft.name !== currentTokenName && nft.marketData?.isTradable),
    )
  }, [allSpaceinvadersBunnyNfts, collectionNfts, currentTokenName])
  ...
  if (isPBCollection) {
    // SpaceinvadersBunnies should display 1 card per bunny id
