import { useRouter } from 'next/router'
import PageLoader from 'components/Loader/PageLoader'
import { isAddress } from 'utils'
import { spaceinvadersBunniesAddress } from '../../constants'
import IndividualSpaceinvadersBunnyPage from './SpaceinvadersBunnyPage'
import IndividualNFTPage from './OneOfAKindNftPage'

const IndividualNFTPageRouter = () => {
  const router = useRouter()
  // For SpaceinvadersBunnies tokenId in url is really bunnyId
  const { collectionAddress, tokenId } = router.query

  if (router.isFallback) {
    return <PageLoader />
  }

  const isPBCollection = isAddress(String(collectionAddress)) === spaceinvadersBunniesAddress
  if (isPBCollection) {
    return <IndividualSpaceinvadersBunnyPage bunnyId={String(tokenId)} />
  }

  return <IndividualNFTPage collectionAddress={String(collectionAddress)} tokenId={String(tokenId)} />
}

export default IndividualNFTPageRouter
