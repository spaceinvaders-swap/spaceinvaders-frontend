...
const ProfilePicture: React.FC = () => {
  ...
  useEffect(() => {
    const fetchUserSpaceinvadersCollectibles = async () => {
      try {
    ...
    if (!isUserNftLoading) {
      ...
      fetchUserSpaceinvadersCollectibles()
    }
  ...
  const handleApprove = async () => {
    ...
    const receipt = await fetchWithCatchTxError(() => {
      return callWithGasPrice(contract, 'approve', [getSpaceinvadersProfileAddress(), selectedNft.tokenId])
    })
    if (receipt?.status) {
    ...
    return (
      <>
        ...
        <Text bold fontSize="20px" mb="24px">
          {t('We couldn’t find any Spaceinvaders Collectibles in your wallet.')}
        </Text>
        <Text as="p" mb="24px">
          {t('Only approved Spaceinvaders Collectibles can be used.')}
          <Link to={`${nftsBaseUrl}/profile/spaceinvaders-collectibles`} style={{ marginLeft: '4px' }}>
            {t('See the list >')}
          </Link>
        </Text>
        <Text as="p">
          {t(
            'You need a Spaceinvaders Collectible to finish setting up your profile. If you sold or transferred your starter collectible to another wallet, you’ll need to get it back or acquire a new one somehow. You can’t make a new starter with this wallet address.',
          )}
        </Text>
      </>
    )
