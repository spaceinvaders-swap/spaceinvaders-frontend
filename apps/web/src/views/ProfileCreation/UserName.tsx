...
import { useSignMessage } from '@offsideswap/wagmi'
...
const UserName: React.FC<React.PropsWithChildren> = () => {
  ...
  const { teamId, selectedNft, userName, actions, minimumRotoRequired, allowance } = useProfileCreation()
  ...
  const hasMinimumRotoRequired = fetchStatus === FetchStatus.Fetched && rotoBalance.gte(REGISTER_COST)
  const [onPresentConfirmProfileCreation] = useModal(
    <ConfirmProfileCreationModal
      ...
      account={account}
      teamId={teamId}
      minimumRotoRequired={minimumRotoRequired}
      allowance={allowance}
    />,
    false,
  )
    ...
    <>
      ...
      {!hasMinimumRotoRequired && (
        <Text color="failure" mt="16px">
          {t('A minimum of %num% ROTO is required', { num: formatUnits(REGISTER_COST) })}
        </Text>
      )}
    </>
  )
