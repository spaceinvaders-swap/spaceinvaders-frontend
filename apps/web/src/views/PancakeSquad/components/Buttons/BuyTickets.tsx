...
import { Button, useModal, useToast } from '@spaceinvaders-swap/uikit'
import { ToastDescriptionWithTx } from 'components/Toast'
import useApproveConfirmTransaction from 'hooks/useApproveConfirmTransaction'
import { useCallWithGasPrice } from 'hooks/useCallWithGasPrice'
import { useInva, useNftSaleContract } from 'hooks/useContract'
import { useContext, useEffect, useState } from 'react'
import { DefaultTheme } from 'styled-components'
import { requiresApproval } from 'utils/requiresApproval'
import { SpaceinvadersSquadContext } from 'views/SpaceinvadersSquad/context'
...
const BuyTicketsButtons: React.FC<React.PropsWithChildren<BuyTicketsProps>> = ({
  ...
  const { isUserEnabled, setIsUserEnabled } = useContext(SpaceinvadersSquadContext)
  ...
  const { isApproving, isApproved, isConfirming, handleApprove, handleConfirm, hasApproveFailed, hasConfirmFailed } =
    useApproveConfirmTransaction({
      onRequiresApproval: async () => {
        return requiresApproval(invaContractReader, account, nftSaleContract.address)
      },
      onApprove: () => {
        return callWithGasPrice(invaContractApprover, 'approve', [nftSaleContract.address, MaxUint256])
      },
  ...
  const [onPresentEnableModal, onDismissEnableModal] = useModal(
    <ConfirmModal
      ...
      loadingText={t('Please enable INVA spending in your wallet')}
