...
const BuyTicketsModal: React.FC<React.PropsWithChildren<BuyTicketsModalProps>> = ({
  ...
  const isRotoBalanceInsufficient = rotoBalance.lt(pricePerTicket)
  const maxBuyTickets = Math.min(rotoBalance.div(pricePerTicket).toNumber(), remainingTickets)
        ...
        <Flex flexDirection="column">
          <Box px="16px">
            ...
            <Flex mb="8px" justifyContent="space-between">
              ...
              <Text font-size="14px">{formatBigNumber(pricePerTicket, 0)} ROTO</Text>
            </Flex>
            <Flex mb="8px" justifyContent="space-between">
              <Text font-size="14px" color="textSubtle">
                {t('Your ROTO Balance')}
              </Text>
              <Text font-size="14px" color={isRotoBalanceInsufficient ? 'failure' : 'text'}>
                {formatBigNumber(rotoBalance, 3)} ROTO
              </Text>
            </Flex>
            <Flex
              mb="8px"
            ...
            <Flex mb="25px" justifyContent="space-between">
              ...
              <Text font-size="14px" bold>
                {formatBigNumber(totalCost, 0)} ROTO
              </Text>
            </Flex>
          </Box>
          <Flex
          ...
          <Box px="16px">
            <Button
              disabled={isRotoBalanceInsufficient}
              onClick={() => buyTicketCallBack({ ticketsNumber })}
              width="100%"
            >
              {isRotoBalanceInsufficient ? t('Insufficient Balance') : t('Confirm')}
            </Button>
          </Box>
        </Flex>
      </ModalBody>
