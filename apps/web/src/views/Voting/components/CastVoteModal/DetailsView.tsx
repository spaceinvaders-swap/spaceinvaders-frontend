...
import { Flex, LinkExternal, Text, Box, HelpIcon, useTooltip, RocketIcon, Link } from '@offsideswap/uikit'
...
const DetailsView: React.FC<React.PropsWithChildren<DetailsViewProps>> = ({
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    <>
      {Number.isFinite(lockedRotoBalance) && (
        <Box>
          <Text>
            {isBoostingExpired
              ? t(
                  'Your vROTO boosting was expired at the snapshot block. Renew your fixed-term staking position to activate the boost for future voting proposals.',
                )
              : t(
                  'Voting power is calculated using the staking amount and remaining staking duration of the fixed-term ROTO staking position at the block.',
                )}
          </Text>
          <Text bold m="10px 0">
            {`${t('ROTO locked:')} ${formatNumber(lockedRotoBalance, 0, 2)}`}
          </Text>
  ...
  return (
    <ModalInner mb="0">
      <Text as="p" mb="24px" fontSize="14px" color="textSubtle">
        {t(
          'Your voting power is determined by the amount of ROTO you held and the remaining duration on the fixed-term staking position (if you have one) at the block detailed below. ROTO held in other places does not contribute to your voting power.',
        )}
      </Text>
      ...
      {Number.isFinite(rotoBalance) && (
        <Flex alignItems="center" justifyContent="space-between" mb="4px">
          <Text color="textSubtle" fontSize="16px">
            {t('Wallet')}
          </Text>
          <Text textAlign="right">{formatNumber(rotoBalance, 0, 3)}</Text>
        </Flex>
      )}
      {Number.isFinite(rotoVaultBalance) && (
        <Flex alignItems="center" justifyContent="space-between" mb="4px">
          <Text color="textSubtle" fontSize="16px">
            {t('Flexible ROTO Staking')}
          </Text>
          <Text textAlign="right">{formatNumber(rotoVaultBalance, 0, 3)}</Text>
        </Flex>
      )}
      {Number.isFinite(rotoPoolBalance) && (
        <>
          {lockedRotoBalance === 0 ? (
            <Flex alignItems="center" justifyContent="space-between" mb="4px">
              <Flex>
                <Text color="textSubtle" fontSize="16px">
                  {t('Fixed Term ROTO Staking')}
                </Text>
              ...
              <Text color="failure" textAlign="right">
                {formatNumber(rotoPoolBalance, 0, 3)}
              </Text>
            </Flex>
          ) : (
      ...
      {Number.isFinite(rotoBnbLpBalance) && (
        <Flex alignItems="center" justifyContent="space-between" mb="4px">
          <Text color="textSubtle" fontSize="16px">
            {t('ROTO BNB LP')}
          </Text>
          <Text textAlign="right">{formatNumber(rotoBnbLpBalance, 0, 3)}</Text>
        </Flex>
      )}
    </ModalInner>
  )
