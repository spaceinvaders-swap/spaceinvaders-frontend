...
import { useLpTokenPrice, usePriceRotoBusd } from 'state/farms/hooks'
import { BIG_ZERO } from '@offsideswap/utils/bigNumber'
import { multicallv2 } from 'utils/multicall'
import { PublicIfoData } from '../../types'
import { getStatus } from '../helpers'

// https://github.com/offsideswap/offside-contracts/blob/master/projects/ifo/contracts/IFOV2.sol#L431
// 1,000,000,000 / 100
...
const useGetPublicIfoData = (ifo: Ifo): PublicIfoData => {
  const { address } = ifo
  const rotoPriceUsd = usePriceRotoBusd()
  const lpTokenPriceInUsd = useLpTokenPrice(ifo.currency.symbol)
  const currencyPriceInUSD = ifo.currency === bscTokens.roto ? rotoPriceUsd : lpTokenPriceInUsd
