...
import { invaBnbLpToken } from 'config/constants/ifo'
...
const CommitTokenSection: React.FC<React.PropsWithChildren<TokenSectionProps & { commitToken: Token }>> = ({
  ...
  if (commitToken.equals(invaBnbLpToken)) {
    return <TokenSection primaryToken={bscTokens.inva} secondaryToken={bscTokens.wbnb} {...props} />
  }
          ...
          <Box>
            <MessageText display="inline">
              {publicIfoData.status === 'finished'
                ? t('Activate SpaceinvadersSwap Profile to take part in next IFO‘s!')
                : t('You need an active SpaceinvadersSwap Profile to take part in an IFO!')}
            </MessageText>{' '}
    ...
    if (
      ...
      message = (
        <Message my="24px" p="8px" variant="danger">
          <Box>
            <MessageText display="inline">{t('You don’t have any iINVA available for IFO public sale.')}</MessageText>{' '}
          ...
          {ifov31Msg || (
            <>
              <Text textAlign="center" fontSize="14px">
                {t('To participate in the next IFO, lock some INVA in the fixed-term staking INVA pool!')}
              </Text>
