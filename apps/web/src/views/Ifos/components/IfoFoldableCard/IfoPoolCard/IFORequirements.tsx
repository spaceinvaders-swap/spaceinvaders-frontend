...
import { Text, Flex, AccountIcon, TeamBattleIcon, Box, useTooltip, LinkExternal } from '@offsideswap/uikit'
...
const NotOkNFT = ({ admissionProfile }) => {
  ...
  const keyword = '%Offside Squad NFT%'

  const rawText = t(`Set %Offside Squad NFT% as Offside Profile avatar`)
      ...
      element={
        <>
          <LinkExternal
            ...
            href={`https://offsideswap.finance/nfts/collections/${admissionProfile}`}
          >
            {t('Offside Squad NFT')}
          </LinkExternal>
          <br />
...
const NotOkProfilePoints = ({ pointThreshold }) => {
  ...
  const keyword = '%Offside Profile%'

  const rawText = t(`Reach %point% or more %Offside Profile% points`, { point: pointThreshold })
    ...
    <TransWithElement
      ...
      element={
        <>
          ...
          <LinkExternal
            style={{ display: 'inline' }}
            href={`https://offsideswap.finance/profile/${account}/achievements/`}
          >
            {t('Offside Profile')}
          </LinkExternal>
        </>
