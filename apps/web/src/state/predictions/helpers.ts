...
import { GRAPH_API_PREDICTION_BNB, GRAPH_API_PREDICTION_ROTO } from 'config/constants/endpoints'
...
import { transformBetResponseROTO, transformUserResponseROTO } from './rotoTransformers'
...
export const transformBetResponse = (tokenSymbol) =>
  tokenSymbol === 'ROTO' ? transformBetResponseROTO : transformBetResponseBNB

export const transformUserResponse = (tokenSymbol) =>
  tokenSymbol === 'ROTO' ? transformUserResponseROTO : transformUserResponseBNB
...
export const getTotalWon = async (): Promise<{ totalWonBNB: number; totalWonROTO: number }> => {
  const [{ market: BNBMarket, market: ROTOMarket }] = await Promise.all([
    request(
  ...
  const totalWonROTO = getTotalWonMarket(ROTOMarket, 'ROTO')

  return { totalWonBNB, totalWonROTO }
}
...
export const getBetHistory = async (
  ...
  api: string,
  tokenSymbol: string,
): Promise<Array<BetResponseBNB | BetResponseROTO>> => {
