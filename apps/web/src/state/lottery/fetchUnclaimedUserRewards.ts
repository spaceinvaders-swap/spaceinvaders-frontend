...
const fetchRotoRewardsForTickets = async (
  winningTickets: LotteryTicket[],
): Promise<{ ticketsWithUnclaimedRewards: LotteryTicket[]; rotoTotal: BigNumber }> => {
  ...
  try {
    const rotoRewards = await multicallv2({ abi: lotteryV2Abi, calls })

    const rotoTotal = rotoRewards.reduce((accum: BigNumber, rotoReward: EthersBigNumber[]) => {
      return accum.plus(new BigNumber(rotoReward[0].toString()))
    }, BIG_ZERO)

    const ticketsWithUnclaimedRewards = winningTickets.map((winningTicket, index) => {
      return { ...winningTicket, rotoReward: rotoRewards[index] }
    })
    return { ticketsWithUnclaimedRewards, rotoTotal }
  } catch (error) {
    console.error(error)
    return { ticketsWithUnclaimedRewards: null, rotoTotal: null }
  }
}
...
export const getWinningTickets = async (
  ...
  if (unclaimedWinningTickets.length > 0) {
    const { ticketsWithUnclaimedRewards, rotoTotal } = await fetchRotoRewardsForTickets(unclaimedWinningTickets)
    return { ticketsWithUnclaimedRewards, allWinningTickets, rotoTotal, roundId }
  }

  if (allWinningTickets.length > 0) {
    return { ticketsWithUnclaimedRewards: null, allWinningTickets, rotoTotal: null, roundId }
  }
