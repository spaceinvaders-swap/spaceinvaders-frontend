...
export const fetchPoolsBlockLimits = async () => {
  ...
  return livePoolsWithEnd.map((invaPoolConfig, index) => {
    const [[startBlock], [endBlock]] = startEndBlockResult[index]
    return {
      sousId: invaPoolConfig.sousId,
...
export const fetchPoolsProfileRequirement = async (): Promise<{
  ...
  const poolProfileRequireCalls = livePoolsWithV3
    .map((validPool) => {
      const contractAddress = getAddress(validPool.contractAddress)
      return ['spaceinvadersProfileIsRequested', 'spaceinvadersProfileThresholdPoints'].map((method) => ({
        address: contractAddress,
        name: method,
      }))
    })
