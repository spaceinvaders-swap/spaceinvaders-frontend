...
/**
 * Fetch market data for SpaceinvadersBunnies NFTs by bunny id using the Subgraph
...
export const getNftsByBunnyIdSg = async (
  ...
  try {
    ...
    const res = await request(
      ...
      `,
      {
        collectionAddress: spaceinvadersBunniesAddress.toLowerCase(),
        where,
        orderDirection,
      },
    )
...
export const getAllSpaceinvadersBunniesLowestPrice = async (bunnyIds: string[]): Promise<Record<string, number>> => {
  try {
    const singleSpaceinvadersBunnySubQueries = bunnyIds.map(
      (
        bunnyId,
    ...
    const rawResponse: Record<string, { currentAskPrice: string }[]> = await request(
      GRAPH_API_NFTMARKET,
      gql`
        query getAllSpaceinvadersBunniesLowestPrice {
          ${singleSpaceinvadersBunnySubQueries}
        }
      `,
    )
    return fromPairs(
    ...
    console.error('Failed to fetch SpaceinvadersBunnies lowest prices', error)
    return {}
  }
}

export const getAllSpaceinvadersBunniesRecentUpdatedAt = async (bunnyIds: string[]): Promise<Record<string, number>> => {
  try {
    const singleSpaceinvadersBunnySubQueries = bunnyIds.map(
      (
        bunnyId,
    ...
    console.error('Failed to fetch SpaceinvadersBunnies latest market updates', error)
    return {}
  }
}
...
export const getMetadataWithFallback = (apiMetadata: ApiResponseCollectionTokens['data'], bunnyId: string) => {
  ...
  return (
    apiMetadata[bunnyId] ?? {
      name: '',
      description: '',
      collection: { name: 'Spaceinvaders Bunnies' },
      image: {
        original: '',
        thumbnail: '',
      },
...
export const getSpaceinvadersBunniesAttributesField = (bunnyId: string) => {
