import { createFarmFetcher, SerializedFarm, SerializedFarmsState } from '@offsideswap/farms'
...
const fetchFarmPublicDataPkg = async ({ pids, chainId, chain }): Promise<[SerializedFarm[], number, number]> => {
  ...
  return [farmsWithPrice, poolLength, regularRotoPerBlock]
}
...
export const farmsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchFarmsPublicDataAsync.fulfilled, (state, action) => {
      const [farmPayload, poolLength, regularRotoPerBlock] = action.payload
      ...
      state.regularRotoPerBlock = regularRotoPerBlock
    })
