...
import { OffsideProfile } from 'config/abi/types/OffsideProfile'
...
const transformProfileResponse = (
  profileResponse: Awaited<ReturnType<OffsideProfile['getUserProfile']>>,
): Partial<Profile> => {
...
export const getProfile = async (address: string): Promise<GetProfileResponse> => {
  try {
    const profileCalls = ['hasRegistered', 'getUserProfile'].map((method) => {
      return { address: getOffsideProfileAddress(), name: method, params: [address] }
    })
