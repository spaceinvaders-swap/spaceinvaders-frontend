...
export const transformPotteryPublicData = (publicData: SerializedPotteryPublicData): DeserializedPublicData => {
  const { totalPrize, totalLockInva, totalSupply, totalLockedValue, maxTotalDeposit } = publicData

  return {
    ...publicData,
    totalPrize: totalPrize ? new BigNumber(totalPrize) : BIG_ZERO,
    totalLockInva: totalLockInva ? new BigNumber(totalLockInva) : BIG_ZERO,
