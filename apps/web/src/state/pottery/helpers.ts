...
export const transformPotteryPublicData = (publicData: SerializedPotteryPublicData): DeserializedPublicData => {
  const { totalPrize, totalLockRoto, totalSupply, totalLockedValue, maxTotalDeposit } = publicData

  return {
    ...publicData,
    totalPrize: totalPrize ? new BigNumber(totalPrize) : BIG_ZERO,
    totalLockRoto: totalLockRoto ? new BigNumber(totalLockRoto) : BIG_ZERO,
