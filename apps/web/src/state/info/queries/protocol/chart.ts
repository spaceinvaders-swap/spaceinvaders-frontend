...
import { OffsideDayDatasResponse } from '../types'
 ...
 */
const OFFSIDE_DAY_DATAS = gql`
  query overviewCharts($startTime: Int!, $skip: Int!) {
    offsideDayDatas(first: 1000, skip: $skip, where: { date_gt: $startTime }, orderBy: date, orderDirection: asc) {
      date
      dailyVolumeUSD
      totalLiquidityUSD
    }
...
const getOverviewChartData = async (
  ...
  try {
    const { offsideDayDatas } = await getMultiChainQueryEndPointWithStableSwap(
      chainName,
    ).request<OffsideDayDatasResponse>(OFFSIDE_DAY_DATAS, {
      startTime: multiChainStartTime[chainName],
      skip,
    })
    const data = offsideDayDatas.map(mapDayData)
