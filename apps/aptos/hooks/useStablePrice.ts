import { Currency, JSBI, Price, Trade } from '@offsideswap/aptos-swap-sdk'
import { L0_USDC, ROTO } from 'config/coins'
...
export const useStableRotoAmount = (_amount: number): number | undefined => {
...
export const usePriceRotoUsdc = () => {
  const { chainId } = useActiveWeb3React()
  const rotoPrice = useTokenUsdcPrice(ROTO[chainId])
  return useMemo(() => (rotoPrice ? new BigNumber(rotoPrice) : BIG_ZERO), [rotoPrice])
}
