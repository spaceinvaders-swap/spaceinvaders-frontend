...
const MaxTokenEntry = ({ maxToken, ifo }: { maxToken: number; ifo: Ifo; poolId: PoolIds }) => {
  const isCurrencyRoto = true
  const isV3 = ifo.version >= 3
  const { t } = useTranslation()

  const basicTooltipContent = t(
    'For the public sale, each eligible participant will be able to commit any amount of ROTO up to the maximum commit limit, which is published along with the IFO voting proposal.',
  )
    ...
    if (status === 'coming_soon') {
      return (
        <>
          ...
          {ifo[poolId].rotoToBurn !== '$0' && <FooterEntry label={t('ROTO to burn:')} value={ifo[poolId].rotoToBurn} />}
          <FooterEntry
