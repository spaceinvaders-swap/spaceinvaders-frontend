...
import { Button, IfoGetTokenModal, useModal, useToast } from '@offsideswap/uikit'
...
const ContributeButton: React.FC<React.PropsWithChildren<Props>> = ({ poolId, ifo, publicIfoData, walletIfoData }) => {
  ...
  const handleContributeSuccess = async (amount: BigNumber, txHash: string) => {
    toastSuccess(
      ...
      <ToastDescriptionWithTx txHash={txHash}>
        {t('You have contributed %amount% ROTO to this IFO!', {
  ...
  const [onPresentGetTokenModal] = useModal(
    <IfoGetTokenModal
      ...
      imageSrc={`https://tokens.offsideswap.finance/images/aptos/${ifo.currency.address}.png`}
    />,
    false,
  )
  ...
  return (
    ...
    >
      ...
      {isMaxCommitted && status === 'live' ? t('Max. Committed') : t('Commit ROTO')}
    </Button>
  )
}
