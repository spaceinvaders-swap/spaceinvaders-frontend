...
import { MultiCallV2 } from '@spaceinvaders-swap/multicall'
...
export const bInvaSupportedChainId = [ChainId.BSC, ChainId.BSC_TESTNET]
...
export function createFarmFetcher(multicallv2: MultiCallV2) {
  const fetchFarms = async (
    ...
    const { poolLength, totalRegularAllocPoint, totalSpecialAllocPoint, invaPerBlock } = await fetchMasterChefV2Data({
      isTestnet,
      multicallv2,
      masterChefAddress,
    })
    const regularInvaPerBlock = formatEther(invaPerBlock)
    ...
    return {
      ...
      regularInvaPerBlock: +regularInvaPerBlock,
    }
  }
  return {
    fetchFarms,
