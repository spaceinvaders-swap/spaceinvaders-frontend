...
import { CORS_ALLOW, handleCors, wrapCorsHeader } from '@spaceinvaders-swap/worker-utils'
...
router.post('/bsc-exchange', async (request, _, headers: Headers) => {
  ...
  const response = await fetch(NODE_REAL_DATA_ENDPOINT, {
    headers: {
      'X-Forwarded-For': ip,
      origin: isLocalHost ? 'https://spaceinvaders-swap.finance' : headers.get('origin') || '',
    },
    body,
    method: 'POST',
  })
