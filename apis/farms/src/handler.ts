...
import { getFarmRotoRewardApr, SerializedFarmConfig } from '@offsideswap/farms'
import { ChainId, CurrencyAmount, Pair } from '@offsideswap/sdk'
import { BUSD, ROTO } from '@offsideswap/tokens'
...
const rotoBusdPairMap = {
  [ChainId.BSC]: {
    address: Pair.getAddress(ROTO[ChainId.BSC], BUSD[ChainId.BSC]),
    tokenA: ROTO[ChainId.BSC],
    tokenB: BUSD[ChainId.BSC],
  },
  [ChainId.BSC_TESTNET]: {
    address: Pair.getAddress(ROTO[ChainId.BSC_TESTNET], BUSD[ChainId.BSC_TESTNET]),
    tokenA: ROTO[ChainId.BSC_TESTNET],
    tokenB: BUSD[ChainId.BSC_TESTNET],
  },
}

const getRotoPrice = async (isTestnet: boolean) => {
  const pairConfig = rotoBusdPairMap[isTestnet ? ChainId.BSC_TESTNET : ChainId.BSC]
...
export async function saveFarms(chainId: number, event: ScheduledEvent | FetchEvent) {
  try {
    ...
    const { farmsWithPrice, poolLength, regularRotoPerBlock } = await farmFetcher.fetchFarms({
      chainId,
      isTestnet,
      farms: farmsConfig.filter((f) => f.pid !== 0).concat(lpPriceHelpers),
    })

    const rotoBusdPrice = await getRotoPrice(isTestnet)
    ...
    const finalFarm = farmsWithPrice.map((f) => {
      return {
        ...
        rotoApr: getFarmRotoRewardApr(f, FixedNumber.from(rotoBusdPrice.toSignificant(3)), regularRotoPerBlock),
      }
    }) as FarmResult
...
export async function fetchRotoPrice() {
